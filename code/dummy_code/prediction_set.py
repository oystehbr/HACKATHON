...
def find_long_time_gaps(df2):
    ...
    for index, pair in enumerate(b.most_common()):
        if pair[1] == 1:
            break
    embed()
    c = b.most_common(26)
    ...
    for i in range(len(sorted_val) - 1):
        ...
        df2['long_time_steps'] = np.where(df2['long_time_steps'].between(sorted_val[i][0], sorted_val[i+1][0]), sorted_val[i+1][1], df2['long_time_steps'])

    # df2['long_time_steps'] = df2['long_time_steps'].between(sorted_val[i+1][0], sorted_val[i+1][0]), sorted_val[i+1][1]*i, df2['long_time_steps'])
    # new thing
    # df2['long_time_steps'] =  df2['long_time_steps']/df2['long_time_steps'].max()
    return df2
...
cinput = input[input["Turbine_Pressure Drafttube"].isna() == False]
...
input_time_array2 = input.index.values.astype(float)
input_time_array1 = input_time_array2 - input_time_array2[0]
input_time_array = input_time_array1/input_time_array1[1]

df['time'] = df_time_array
df['time_s'] = df_time_array**2

input['time'] = input_time_array
input['time_s'] = input_time_array**2
...
input = find_long_time_gaps(input)
...
if not on_off:
    ...
    "time", "time_s"]].iloc[split:]
    input = input[["Unit_4_Power", "Unit_4_Reactive Power", "Turbine_Guide Vane Opening", "Turbine_Pressure Drafttube", "Turbine_Pressure Spiral Casing", "Turbine_Rotational Speed", \
    "time", "time_s"]]
else:
    X_train = df[["Unit_4_Power", "Turbine_Pressure Drafttube", "Turbine_Pressure Spiral Casing", "Turbine_Rotational Speed", \
    "time", "time_s", "long_time_steps"]].iloc[:split]
    X_test = df[["Unit_4_Power", "Turbine_Pressure Drafttube", "Turbine_Pressure Spiral Casing", "Turbine_Rotational Speed", \
    "time", "time_s", "long_time_steps"]].iloc[split:]
    input = input[["Unit_4_Power", "Turbine_Pressure Drafttube", "Turbine_Pressure Spiral Casing", "Turbine_Rotational Speed", \
    "time", "time_s", "long_time_steps"]]



y_train = df["Bolt_3_Tensile"].iloc[:split]
y_test = df["Bolt_3_Tensile"].iloc[split:]
...
pred_predicion_set = logreg.predict(input)
...
plt.legend(); plt.show(); plt.close()
plt.plot(pred_predicion_set, label="pred prediction_data");
plt.legend(); plt.show(); plt.close()
print(error_test)
print(logreg.coef_)
